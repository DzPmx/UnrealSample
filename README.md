...
- RGB 保存 Base Color。
- Single Billboard、Cross Cards 和 BillboardClouds 的 A 通道保存可见源材质分类：背景为 `0`、树干为 `0.5`（8-bit 值 `128`）、树叶为 `1`（8-bit 值 `255`），不生成 SDF。
...
Single Billboard、Cross Cards 和 BillboardClouds 可以用低于树干值的阈值计算覆盖，并同时保留分类：
...
当 Single Billboard、Cross Cards 或 BillboardClouds 关闭 Mix 输出时，插件不生成 Mix 纹理，而是继续从相同的深度胜出片元读取 Roughness 和 Specular。像素按源三角形的树干/树叶分类分别计算算术平均值，并写入生成材质实例的四个 Scalar Parameter：
...
- Single Billboard 和 Cross Cards 始终按每个视角的原始有效覆盖范围裁切，`Per-View Alpha Crop Guard` 单独控制额外保留边界。
...
- 默认类型后缀为：Single Billboard 使用 `_Billboard`，Cross Cards 使用 `_Cross`，Impostor 使用 `_Impostor`，BillboardClouds 使用 `_BillboardClouds`。纹理通道标记继续位于类型之后，例如 `_Cross_DA`、`_Cross_NR` 和 `_Cross_M`。
