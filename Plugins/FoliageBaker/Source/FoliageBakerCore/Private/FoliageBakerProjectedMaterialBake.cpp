...
namespace UE::FoliageBaker::ProjectedMaterialBake
{
	namespace
	{
		...
		FVector DecodeNormalColor(const FColor& Color)
		{
			return FVector(
				...
				static_cast<double>(Color.B) / 255.0 * 2.0 - 1.0)
		...
		FNormalBasisSample MakeNormalBasisSample(
			const PlaneCover::FSourceTriangle& Triangle,
			const double W0,
			const double W1,
			const double W2,
			...
			TArray<int32>* OutRasterSourceTriangleIndices)
		{
				...
				const FVector Positions[3]) -> bool
			{
				...
				for (int32 Corner = 0; Corner < 3; ++Corner)
				{
					...
					const FNormalBasisSample Basis = MakeNormalBasisSample(Triangle, W0, W1, W2, false);
		...
		bool BuildNormalBasisMap(
			...
			const FPlaneSideBakeParams& Params,
			const bool bEffectiveReverseWinding,
			TArray<FNormalBasisSample>& OutBasisMap)
		{
			OutBasisMap.Reset();
			if (Params.TileSize.X <= 0 || Params.TileSize.Y <= 0)
			{
				return false;
			}

			OutBasisMap.Init(FNormalBasisSample(), Params.TileSize.X * Params.TileSize.Y);
			const bool bFlipTextureV = Params.AtlasVConvention == PlaneCover::EAtlasVConvention::GeometryMinVToTextureMaxV;
			const double UExtent = FMath::Max(PlaneInfo.MaxU - PlaneInfo.MinU, UE_DOUBLE_SMALL_NUMBER);
			const double VExtent = FMath::Max(PlaneInfo.MaxV - PlaneInfo.MinV, UE_DOUBLE_SMALL_NUMBER);
				...
				const FVector Positions[3])
			{
				...
				const bool bFlipOutputNormalForTwoSidedBackFace = Params.bFlipTwoSidedBackFaceOutputNormals
					&& FVector::DotProduct(Triangle.Normal, Params.CaptureRayDirection) < 0.0;
				...
				for (int32 VertexIndex = 0; VertexIndex < 3; ++VertexIndex)
				{
					...
					const FVector ProjectedVertex = PlaneCover::ProjectPointToPlane(
						Positions[VertexIndex], PlaneInfo.Normal, PlaneInfo.Rho);
					...
					ProjectedPoints[VertexIndex] = FVector2D(
						UFraction * Params.TileSize.X,
						TileVFraction * Params.TileSize.Y);
				}
				...
				const int32 MinX = FMath::Clamp(FMath::FloorToInt(FMath::Min3(ProjectedPoints[0].X, ProjectedPoints[1].X, ProjectedPoints[2].X)), 0, Params.TileSize.X - 1);
				const int32 MaxX = FMath::Clamp(FMath::CeilToInt(FMath::Max3(ProjectedPoints[0].X, ProjectedPoints[1].X, ProjectedPoints[2].X)), 0, Params.TileSize.X - 1);
				const int32 MinY = FMath::Clamp(FMath::FloorToInt(FMath::Min3(ProjectedPoints[0].Y, ProjectedPoints[1].Y, ProjectedPoints[2].Y)), 0, Params.TileSize.Y - 1);
				const int32 MaxY = FMath::Clamp(FMath::CeilToInt(FMath::Max3(ProjectedPoints[0].Y, ProjectedPoints[1].Y, ProjectedPoints[2].Y)), 0, Params.TileSize.Y - 1);
				...
				for (int32 Y = MinY; Y <= MaxY; ++Y)
				{
					for (int32 X = MinX; X <= MaxX; ++X)
					{
						...
						const int32 PixelIndex = Y * Params.TileSize.X + X;
						if (!OutBasisMap.IsValidIndex(PixelIndex))
						{
							continue;
						}

						FNormalBasisSample BasisSample = MakeNormalBasisSample(
							Triangle,
							SourceW0,
							SourceW1,
							SourceW2,
						...
						BasisSample.CaptureDepth = FVector::DotProduct(SourcePoint, Params.CaptureRayDirection);
		...
		TArray<int32>* OutRasterSourceTriangleIndices)
	{
		...
		if (Params.TileSize.X <= 0 || Params.TileSize.Y <= 0)
		{
			if (OutError)
			{
				*OutError = TEXT("Projected material bake tile size must be positive.");
			}
			return false;
		}
		...
		const bool bFlipTextureV = Params.AtlasVConvention == PlaneCover::EAtlasVConvention::GeometryMinVToTextureMaxV;
		const bool bEffectiveReverseWinding = Params.bBackSide != bFlipTextureV;
		...
		if (Params.bBuildNormalBasisMap && !BuildNormalBasisMap(
				Triangles,
				SortedTriangleIndices,
				SortedCrackReductionProjections,
				PlaneInfo,
				...
				OutNormalBasisMap))
		{
			// A sub-pixel projected triangle may build valid material geometry without
			// covering a basis-map pixel center. Color/coverage callers can still bake
			// that geometry; normal callers must explicitly require a non-empty map.
			OutNormalBasisMap.Reset();
		}
		else if (!Params.bBuildNormalBasisMap)
		{
			OutNormalBasisMap.Reset();
		}
		return true;
		...
		const uint8 AlphaOverride)
	{
		...
		const FVector TangentSpaceNormal = DecodeNormalColor(RawBakedTangentSpaceNormal);
		...
		const FVector ObjectSpaceNormal = (Tangent * TangentSpaceNormal.X
			...
			.GetSafeNormal(UE_DOUBLE_SMALL_NUMBER, Normal)
