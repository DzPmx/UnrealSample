...
public:
	...
	UPROPERTY(config, EditAnywhere, Category = "Feature|Trunk Cards", meta = (ClampMin = "2", ClampMax = "4", EditCondition = "bEnableTrunkCards", EditConditionHides, ToolTip = "Number of vertical trunk cross-card planes. 2 is a cross card, 3 is a three-way star, 4 is a four-way star."))
	int32 TrunkCardPlaneCount = 4;

	UPROPERTY(config, EditAnywhere, Category = "Feature|Trunk Cards", meta = (EditCondition = "bEnableTrunkCards", EditConditionHides, ToolTip = "Resolution weight for trunk cross-card atlas tiles during packing. The atlas resolution stays fixed, so larger trunk tiles reduce space available to foliage billboard tiles."))
